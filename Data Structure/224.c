...
int if_boss( Employee *set, int n, Employee *employee, Employee *target ){
    int i=0;
    while(i<n){
        if(employee->boss_id == employee->id) return 0;
        if(employee->boss_id == target->id) return 1;
        employee = find_employee_by_id(set, n, employee->boss_id);
        i++;
    }
    return 0;
...
int check_relationship(Employee *set, int n, Employee *A, Employee *B){
    if(if_boss(set, n, A, B)) return 1;
    if(if_boss(set, n, B, A)) return 2;
